    ...
    <nav className="sticky top-0 z-50 w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* LOGO */}
          <div onClick={() => router.push(NAVBAR_CONFIG.LOGO.PATH)} className="flex items-center gap-2 group cursor-pointer">
            ...
            <span className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              {NAVBAR_CONFIG.LOGO.TITLE}<span className="text-blue-600">{NAVBAR_CONFIG.LOGO.SUBTITLE}</span>
            </span>
          </div>
          ...
          <div className="relative">
            ...
            {isMenuOpen && (
              <>
                ...
                <div className="absolute right-0 mt-3 w-72 bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 py-4 z-20 animate-in fade-in zoom-in duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-3 px-4 mt-4">
                    {NAVBAR_CONFIG.MENU_ITEMS.map((item) => (
                      ...
                      >
                        ...
                        <p className="text-[11px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-tighter">{item.label}</p>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 px-4">
                    <button className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors">
                      <span>{NAVBAR_CONFIG.LOGOUT.LABEL}</span>
                    </button>
                  </div>
                </div>
              </>
